...
class POIMapViewModel extends ChangeNotifier {
  ...
  Future<void> loadPOIData({String? initialBuilding, String? initialFloor}) async {
    ...
    try {
      ...
      if (location == null) {
        _setError('Could not determine your location. Please check location permissions.');
        return;
      }
  ...
  Future<void> _useInitialBuildingAndFloor(String initialBuilding, String? initialFloor) async {
  ...
  Future<void> retry({String? initialBuilding, String? initialFloor}) async {
    await loadPOIData(initialBuilding: initialBuilding, initialFloor: initialFloor);
  }
  ...
  void panToPOI(POI poi, Size viewportSize) {
    ...
    _indoorMapViewModel.centerOnPoint(poiPosition, viewportSize, padding: 100.0);
  }
