...
class UserCalendarEvent {
  ...
  UserCalendarEvent(this.userCalendar, this.eventId, this.description, this.localStart, this.localEnd, this.title, this.locationField);
}
...
class CalendarRepository {
  ...
  Future<List<UserCalendar>> getUserCalendars() async {
    ...
    for (var systemUserCalendar in systemUserCalendars.data as UnmodifiableListView<Calendar>) {
      // Calendars without IDs cannot be referred to in future calls
      if (systemUserCalendar.id != null) {
        returnData.add(UserCalendar(systemUserCalendar.id!, systemUserCalendar.name));
      }
    }
   ...
   */
  Future<List<UserCalendarEvent>> getEvents(List<UserCalendar> selectedCalendars, Duration timeSpan, DateTime? utcStart) async {
    ...
    for (var userCalendar in selectedCalendars) {
      var systemCalendarEvents = await plugin.retrieveEvents(userCalendar.calendarId, RetrieveEventsParams(startDate: startDate, endDate: endDate));
      if (!systemCalendarEvents.isSuccess) continue;
      for (var systemCalendarEvent in systemCalendarEvents.data!) {
        if ((systemCalendarEvent.allDay != null && systemCalendarEvent.allDay == true)
        ...
        returnData.add(UserCalendarEvent(userCalendar, systemCalendarEvent.eventId!, systemCalendarEvent.description, systemCalendarEvent.start!.toLocal(), systemCalendarEvent.end!.toLocal(), systemCalendarEvent.title!, systemCalendarEvent.location));
      }
    }
  ...
  /**
   * Helper method to get events on a given local date from today. Pass 0 to get today's
   * events.
   */
  Future<List<UserCalendarEvent>> getEventsOnLocalDate(List<UserCalendar> selectedCalendars, int offset) async {
