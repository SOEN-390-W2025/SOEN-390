...
// Wait time between Floor 1 and Floor 2 should be 10 + (5 * 1) = 15 seconds
  ...
  group('ConcordiaRoom tests', () {
    ...
    test('check override for == operator works', () {
      ...
      const RoomCategory category = RoomCategory.auditorium;
      final room = ConcordiaRoom(roomNumber, category, mockFloor, entrance);
      final room2 = ConcordiaRoom(roomNumber, RoomCategory.classroom, mockFloor, entrance);
  ...
  test('getDistanceBetweenPoints should calculate the correct distance between two points', () {
    final point1 =
  ...
  test('getDistanceBetweenPoints should return zero distance for the same points', () {
    final point1 =
        ConcordiaFloorPoint(ConcordiaFloor("1", BuildingRepository.h), 0, 0);
    final point2 =
        ConcordiaFloorPoint(ConcordiaFloor("1", BuildingRepository.h), 0, 0);
  ...
  test('getFloorRoutablePointListTravelTime should calculate the correct travel time between points', () {
    final point1 =
        ConcordiaFloorPoint(ConcordiaFloor("1", BuildingRepository.h), 0, 0);
    final point2 =
        ConcordiaFloorPoint(ConcordiaFloor("2", BuildingRepository.h), 3, 4);
    ...
    final result =
        IndoorRoute(BuildingRepository.h, null, null, null, null, null, null, null)
          .getFloorRoutablePointListTravelTime(points);

    expect(result, 5.0);
  });

  test('getFloorRoutablePointListTravelTime should return 0 when only one point in list', () {
    final point1 =
        ConcordiaFloorPoint(ConcordiaFloor("1", BuildingRepository.h), 0, 0);
    final points = [point1];
